package com.example.azsserver.appuser;
...
public class UserService implements UserDetailsService {
    ...
    public AppUser authUser(AppUser requestAppUser) {
        AppUser user;
        user = userRepository.findUserByUsername(requestAppUser.getUsername())
                .orElse(new AppUser("", ""));
        if (Objects.equals(user.getUsername(), requestAppUser.getUsername())
                && (new BCryptPasswordEncoder()).matches(requestAppUser.getPassword(), user.getPassword()))
            return user;
        else throw new IllegalStateException("incorrect username or password");
    }
